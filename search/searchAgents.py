...
def cornersHeuristic(state, problem):
  ...
  heuristic = 0;
  rankQueue = util.PriorityQueue();
  leftToReach = [];
  for i in range(0, 4):
      if (not goal[i]):
          leftToReach.append(corners[i]);
  length = len(leftToReach);
  if (length != 0):
      import itertools;
      allPossibleOrdering = itertools.permutations(leftToReach, length);
      for state in allPossibleOrdering:
          temp = util.manhattanDistance(currentPosition, state[0]);
          for i in range(1, length-1):
              temp += util.manhattanDistance(state[i], state[i+1]);
          rankQueue.push(temp, temp);
      heuristic = rankQueue.pop();
...
def foodHeuristic(state, problem):
  ...
  output = [];
  totalLength = 0;
  sizeOfVertexes = len(vertexes);
  while (len(output) < sizeOfVertexes - 1):
      edge = edges.pop();
      u, v = edge[0];
      w = edge[1];
      for eachSet in sets:
